...
import { anonymizeClientAdmin, updateClientAdmin } from '@/app/admin/actions';

export function ClientDetailPanel({ client }: {
  client: { id: string; full_name: string; phone: string | null; email: string | null; is_disabled: boolean; notes: string };
}) {
  const router = useRouter();
  const [values, setValues] = useState({ full_name: client.full_name, phone: client.phone ?? '', email: client.email ?? '', notes: client.notes });
    ...
    <section className="admin-panel" aria-label="بيانات العميل">
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          run(() => updateClientAdmin(client.id, { full_name: values.full_name, phone: values.phone || null, email: values.email || null, notes: values.notes || null }));
        }}
      >
        ...
        <div className="admin-action-row">
          ...
          {client.is_disabled
            ? <button type="button" className="admin-btn" disabled={pending} onClick={() => run(() => updateClientAdmin(client.id, { is_disabled: false }))}><ShieldCheck aria-hidden="true" /> إعادة تفعيل</button>
            : <button type="button" className="admin-btn" disabled={pending} onClick={() => run(() => updateClientAdmin(client.id, { is_disabled: true }), 'تعطيل حساب هذا العميل؟')}><ShieldBan aria-hidden="true" /> تعطيل الحساب</button>}
          <button
            type="button" className="admin-btn admin-row-btn--danger" disabled={pending}
            onClick={() => run(
              () => anonymizeClientAdmin(client.id),
              'سيتم إخفاء البيانات الشخصية للعميل نهائياً مع الاحتفاظ بسجل طلباته المالية. متابعة؟',
            )}
          >
            <Trash2 aria-hidden="true" /> حذف/إخفاء الهوية
          </button>
        </div>
      </form>
    </section>
