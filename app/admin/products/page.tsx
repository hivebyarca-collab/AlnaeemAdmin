...
import { getBrands, getCategories, listAdminProducts } from '@/lib/database';
...
export default async function ProductListPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  let data: Awaited<ReturnType<typeof listAdminProducts>> | null = null;
  ...
  return (
    <div className="admin-page-body">
      <header className="admin-page-header">
        <h1><span aria-hidden="true">📦</span> قائمة المنتجات</h1>
        <p>كل ما في متجرك في مكان واحد — ابحث، صفِّ، وعدّل بضغطة.</p>
      </header>

      <form className="admin-filter-bar">
        <Link href="/admin/products/new" className="admin-btn admin-btn--primary"><Plus aria-hidden="true" /> إضافة منتج جديد</Link>
      ...
      {data.rows.length === 0 ? (
        <div className="admin-empty">
          ...
          <p>جرّب تغيير عوامل التصفية أو أضف منتجاً جديداً.</p>
          <Link href="/admin/products/new" className="admin-btn admin-btn--primary">إضافة منتج</Link>
        </div>
      ) : (
        <div className="admin-panel admin-table-panel">
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead>
                <tr>
                  <th>صورة المنتج</th><th>اسم المنتج</th><th>العلامة / الموديل</th><th>SKU</th>
                  <th>السعر</th><th>المخزون</th><th>الحالة</th><th>الإجراءات</th>
                </tr>
              </thead>
              <tbody>
                {data.rows.map((product) => {
                  ...
                  return (
                    <tr key={product.id}>
                      ...
                      <td><Link href={`/admin/products/${product.id}`} className="admin-table-title">{product.name}</Link></td>
          ...
          <footer className="admin-pagination">
            ...
            <nav aria-label="صفحات المنتجات">
              {Array.from({ length: Math.min(totalPages, 7) }, (_, index) => index + 1).map((pageNumber) => (
                <Link
                  key={pageNumber} href={`/admin/products?${new URLSearchParams({ ...(filters as Record<string, string>), page: String(pageNumber) })}`}
