...
export async function saveProduct(values: ProductFormValues): Promise<ActionResult> {
  ...
  const existing = productService.getProductBySku(sku);
  if (existing && existing.id !== values.id) return { ok: false, error: 'رمز المنتج (SKU) مستخدم مسبقاً — يجب أن يكون فريداً' };

  const existingProduct = values.id ? productService.getProduct(values.id) : undefined;
  ...
  if (productId) {
    productService.updateProduct(productId, base);
  } else {
    productId = productService.createProduct(base).id;
  }
  productService.updateProductExtended(productId, {
...
export async function setProductActive(id: string, isActive: boolean): Promise<ActionResult> {
  await requireAdmin();
  if (!productService.updateProduct(id, { is_active: isActive ? 1 : 0 })) return { ok: false, error: 'المنتج غير موجود' };
  revalidatePath('/products');
...
export async function removeProduct(id: string): Promise<ActionResult> {
  await requireAdmin();
  if (!productService.getProduct(id)) return { ok: false, error: 'المنتج غير موجود' };
...
export async function updateOrderDetails(id: string, changes: { status?: string; payment_status?: string; notes?: string }): Promise<ActionResult> {
  ...
  const updated = orderService.updateOrder(id, changes as { status?: 'SUBMITTED' | 'CONFIRMED' | 'PREPARING' | 'READY' | 'COMPLETED' | 'CANCELLED'; payment_status?: 'unpaid' | 'paid' | 'refunded'; notes?: string });
...
export async function updateCustomerAdmin(id: string, changes: { full_name?: string; email?: string | null; phone?: string | null; notes?: string | null; is_disabled?: boolean }): Promise<ActionResult> {
  ...
  if (!customerService.updateCustomer(id, changes)) return { ok: false, error: 'العميل غير موجود' };
...
export async function anonymizeCustomerAdmin(id: string): Promise<ActionResult> {
  await requireAdmin();
  customerService.anonymizeCustomer(id);
  revalidatePath('/customers');
  revalidatePath('/');
...
export async function setConversationAiState(id: string, aiPaused: boolean): Promise<ActionResult> {
  await requireAdmin();
  conversationService.setFlags(id, { ai_paused: aiPaused, needs_admin: aiPaused });
  revalidatePath('/customers');
